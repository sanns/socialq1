...
task('frontend', [], () => {
  ...
  exec('cd frontend && set NODE_ENV=development ../node_modules/.bin/webpack', (error, stdout, stderr) => {
    if (error) {
...
task('db.create', ['configs'], () => {
  ...
  const database = config.database || 'm3gd1';
  const conf2 = {
      user: 'postgres',
      password: 'root',
      port: 5432,
  ...
  pgtools.createdb(conf2, database, function (err, res) {
    if (err) {
      console.log('Ошибка sanz ', err.name, conf);
      console.log('config ', __dirname, config);
...
task('migrate', ['configs'], () => {
  ...
  exec(`set NODE_ENV=${env} npm run migrate`, (error, stdout, stderr) => {
    if (error) {
...
task('run.dev', ['depends.dev'], () => {
  ...
  service.stdout.on('data', data => { process.stdout.write(`sanz.run.dev ${data}`) });
  service.stderr.on('data', data => { process.stdout.write(`sanz.run.dev ${data}`) });
  service.on('close', code => { process.stdout.write(`sanz.run.dev Ошибка ${code}`) });
}, true);
